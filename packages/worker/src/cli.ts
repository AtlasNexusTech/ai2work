...
async function main(){const command=process.argv[2]; if(!command||command==='help'){console.log(['aijobs-worker evaluate --bounty file [--policy file] [--ledger file]','aijobs-worker prepare --bounty file --policy file --workspace dir --ledger file','aijobs-worker verify-ledger --ledger file'].join('\n'));return;}
 const ledgerPath=arg('--ledger')??'.aijobs/ledger.jsonl'; const ledger=new AuditLedger(resolve(ledgerPath));
